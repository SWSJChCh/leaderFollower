...
#Proportion of domain for which Colec12 is expressed
Colec12Len = int(float(sys.argv[1]) * lengthList[-1] / meshScale)
...
for _ in range(repeats):
    ...
    halfPadded = np.vstack((centerArray, createDANArray(meshWit//3, meshLen, \
                           c0, bcParam, Colec12Len)))
    Colec12Mesh = np.vstack((createDANArray(meshWit//3, meshLen, \
                           c0, bcParam, Colec12Len), halfPadded))
    ...
    DANArray = cv2.resize(DANMesh, (Len, Wit), \
                           interpolation = cv2.INTER_AREA)
    #Initialise DAN Array (for cells)
    Colec12Array = cv2.resize(Colec12Mesh, (Len, Wit), \
                           interpolation = cv2.INTER_AREA)
        ...
        if t > firstInsert:
            ...
            for _ in range(subStep):
                ...
                DANMesh = updateDAN(DANMesh, D, chi, lmda, cellRad, \
                                      cellList, dt, subStep, searchRad, \
                                      Len, Wit, lenDot, meshScale)
                #Update Colec12 (Not degraded)
                Colec12Mesh = updateColec12(Colec12Mesh, D, chi, 0, cellRad, \
                                      cellList, dt, subStep, searchRad, \
                                      Len, Wit, lenDot, meshScale)
            #Update VEGF Array
            VEGFArray = cv2.resize(VEGFMesh, (Len, Wit), \
            ...
            DANArray = cv2.resize(DANMesh, (Len, Wit), \
                                   interpolation = cv2.INTER_AREA)
            #Update Colec12 Array
            Colec12Array = cv2.resize(Colec12Mesh, (Len, Wit), \
                                   interpolation = cv2.INTER_AREA)

            #Move cells
            filList = moveCells(VEGFArray, DANArray, Colec12Array, cellList, \
            ...
            if counter % animStep == 0 and animate:
                ...
                Colec12Mast.append(Colec12Array.copy())
...
if animate:
    ...
    for i in range(len(cellMast)):
        ...
        #Show DAN Profile
        im1 = ax[1].imshow(Colec12Mast[i], interpolation = 'none', vmin = 0, \
                           vmax = np.amax(Colec12Mast[i]))
        #Title
