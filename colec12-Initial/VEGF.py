...
'''
Calculate and return an updated COLEC12 matrix from the COLEC12 PDE
'''
def updateColec12(Colec12Array, D, chi, lmbd, R, posList, dt, subStep, \
               searchRad, L, W, Ldot, meshScale):
    #Updated VEGF array calculated by Taylor expansion
    #c(x, t + delta t) = c(x, t) + delta t * c'(x, t)
    Colec12Array = Colec12Array + \
            np.multiply(dt / subStep, diffusion(Colec12Array, D, L, meshScale)) - \
            np.multiply(dt / subStep, dilution(Colec12Array, L, Ldot)) + \
            np.multiply(dt / subStep, logistic(Colec12Array, chi))
    #Zero-flux boundary conditions
    Colec12Array[:, 0] = Colec12Array[:, 1]
    Colec12Array[:, -1] = Colec12Array[:, -2]
    #Updated VEGF array with zero-flux boundary conditions
    return Colec12Array
