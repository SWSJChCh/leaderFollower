'''
VEGF.py - Samuel Johnson - 27/05/24
'''
...
'''
Calculate and return an updated Colec12Trail matrix from the Colec12Trail PDE
'''
def updateColec12Trail(Colec12TrailArray, D, chi, lmbd, R, posList, dt, subStep, \
               searchRad, L, W, Ldot, meshScale):
    #Updated VEGF array calculated by Taylor expansion
    #c(x, t + delta t) = c(x, t) + delta t * c'(x, t)
    Colec12TrailArray = Colec12TrailArray + \
            np.multiply(dt / subStep, diffusion(Colec12TrailArray, D, L, meshScale)) - \
            np.multiply(dt / subStep, dilution(Colec12TrailArray, L, Ldot)) + \
            np.multiply(dt / subStep, logistic(Colec12TrailArray, chi))
    #Zero-flux boundary conditions
    Colec12TrailArray[:, 0] = Colec12TrailArray[:, 1]
    Colec12TrailArray[:, -1] = Colec12TrailArray[:, -2]
    #Updated VEGF array with zero-flux boundary conditions
    return Colec12TrailArray
