...
#Proportion of domain for which Trail is expressed
TrailLen = int(float(sys.argv[1]) * lengthList[-1] / meshScale)
...
for _ in range(repeats):
    ...
    halfPadded = np.vstack((centerArray, createDANArray(meshWit//3, meshLen, \
                           c0, bcParam, TrailLen)))
    TrailMesh = np.vstack((createDANArray(meshWit//3, meshLen, \
                           c0, bcParam, TrailLen), halfPadded))
    ...
    DANArray = cv2.resize(DANMesh, (Len, Wit), \
                           interpolation = cv2.INTER_AREA)
    #Initialise DAN Array (for cells)
    TrailArray = cv2.resize(TrailMesh, (Len, Wit), \
                           interpolation = cv2.INTER_AREA)
        ...
        if t > firstInsert:
            ...
            for _ in range(subStep):
                ...
                DANMesh = updateDAN(DANMesh, D, chi, lmda, cellRad, \
                                      cellList, dt, subStep, searchRad, \
                                      Len, Wit, lenDot, meshScale)
                #Update Trail (Not degraded)
                TrailMesh = updateTrail(TrailMesh, D, chi, 0, cellRad, \
                                      cellList, dt, subStep, searchRad, \
                                      Len, Wit, lenDot, meshScale)
            #Update VEGF Array
            VEGFArray = cv2.resize(VEGFMesh, (Len, Wit), \
            ...
            #Move cells
            filList = moveCells(VEGFArray, DANArray, TrailArray, cellList, \
            ...
            if counter % animStep == 0 and animate:
                ...
                TrailMast.append(TrailArray.copy())
...
if animate:
    ...
    for i in range(len(cellMast)):
        ...
        #Show DAN Profile
        im1 = ax[1].imshow(TrailMast[i], interpolation = 'none', vmin = 0, \
                           vmax = np.amax(TrailMast[i]))
        #Title
    ...
    with imageio.get_writer('LeaderFollowerTrailConfinement{}.mp4'.\
