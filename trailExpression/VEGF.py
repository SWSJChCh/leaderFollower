...
'''
Calculate and return an updated Trail matrix from the Trail PDE
'''
def updateTrail(TrailArray, D, chi, lmbd, R, posList, dt, subStep, \
               searchRad, L, W, Ldot, meshScale):
    #Updated VEGF array calculated by Taylor expansion
    #c(x, t + delta t) = c(x, t) + delta t * c'(x, t)
    TrailArray = TrailArray + \
            np.multiply(dt / subStep, diffusion(TrailArray, D, L, meshScale)) - \
            np.multiply(dt / subStep, dilution(TrailArray, L, Ldot)) + \
            np.multiply(dt / subStep, logistic(TrailArray, chi))
    #Zero-flux boundary conditions
    TrailArray[:, 0] = TrailArray[:, 1]
    TrailArray[:, -1] = TrailArray[:, -2]
    #Updated VEGF array with zero-flux boundary conditions
    return TrailArray
